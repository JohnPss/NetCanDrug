...
# This script is in src/pipeline, so two levels up from __file__ is src, three is project root.
# We want to add the project root (NetCanDrug) to sys.path so we can do 'from src.config import config'
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if project_root not in sys.path:
    sys.path.insert(0, project_root)

try:
    # Try importing config as a module from src.config package
    from src.config import config
except ImportError:
    # Fallback: if running from src/pipeline directly and path is somehow messed up
    import config
...
def normalize_df_column(df, col):
    # return normalized column (0-1) manually
    if df[col].nunique() > 1:
        min_val = df[col].min()
        max_val = df[col].max()
...
def main():
    ...
    df_topology['degree_norm'] = normalize_df_column(df_topology, 'degree')
    df_topology['betweenness_norm'] = normalize_df_column(df_topology, 'betweenness_centrality')
    ...
    for drug in unique_drugs: # tqdm removed
